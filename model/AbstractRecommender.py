...
class AbstractRecommender(object):
    ...
    def predict2(self, user_ids, items, owner):
        raise NotImplementedError
...
# class OwnerAbstractRecommender(AbstractRecommender):
#     def __init__(self, dataset, conf):
#         super(OwnerAbstractRecommender, self).__init__(dataset, conf)
#         item_owner = pd.read_csv(conf["owner_file"], sep=conf["data.convert.separator"],
#                                  header=None, names=["item", "owner"])
#         items_key = np.array(list(dataset.itemids.keys()))
#         index = np.in1d(item_owner["item"], items_key)
#         item_owner = item_owner[index]
#
#         owners_key = np.array(list(dataset.userids.keys()))
#         index = np.in1d(item_owner["owner"], owners_key)
#         item_owner = item_owner[index]
#
#         item = item_owner["item"]
#         item_id = [dataset.itemids[i] for i in item]
#         owner = item_owner["owner"]
#         owner_id = [dataset.userids[c] for c in owner]
#         num_owners, num_items = dataset.train_matrix.shape
#         self.own_matrix = sp.csr_matrix(([1] * len(item_id), (item_id, owner_id)),
#                                         shape=(num_items, num_owners))

class OwnerAbstractRecommender(AbstractRecommender):
    def __init__(self, dataset, conf):
        super(OwnerAbstractRecommender, self).__init__(dataset, conf)
        self.all_ownerIDs = np.ones(dataset.num_items,dtype=np.int32)
        # print("开始生成owner矩阵：%f" % time.time())
        with open(conf["owner_file"], 'r') as f1:
            lines = f1.readlines()
            for entry in range(len(lines)):
                item_outerID, owner_outerID = lines[entry].strip().split(',')
                # print("item_outerID:",item_outerID)
                # print("owner_outerID:",owner_outerID)
                # print("itemids:",dataset.itemids[int(item_outerID)])
                # print("userids:",dataset.userids[int(owner_outerID)])
                self.all_ownerIDs[dataset.itemids[int(item_outerID)]] = dataset.userids[int(owner_outerID)]


        # print("生成owner矩阵：%f" % time.time())
        # print("id13的owner：%f" % self.all_ownerIDs[13])


class UIOAbstractRecommender(AbstractRecommender):
    def __init__(self, dataset, conf):
        super(UIOAbstractRecommender, self).__init__(dataset, conf)
        self.all_ownerIDs = np.ones(dataset.num_items, dtype=np.int32)
        with open(conf["owner_file"], 'r') as f1:
            lines = f1.readlines()
            for entry in range(len(lines)):
        ...
        social_users = pd.read_csv(conf["realtion_file"], sep=conf["data.convert.separator"],
                                   header=None, names=["user", "owner"])
        users_key = np.array(list(dataset.userids.keys()))
        index = np.in1d(social_users["user"], users_key)
        social_users = social_users[index]

        index = np.in1d(social_users["owner"], users_key)
        social_users = social_users[index]

        user = social_users["user"]
        user_id = [dataset.userids[u] for u in user]
        owner = social_users["owner"]
        owner_id = [dataset.userids[u] for u in owner]
        num_users, num_items = dataset.train_matrix.shape
        self.social_matrix = sp.csr_matrix(([1] * len(user_id), (user_id, owner_id)),
                                           shape=(num_users, num_users))